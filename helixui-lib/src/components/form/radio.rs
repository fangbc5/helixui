...
use crate::components::focus::{
    use_focus_controlled_item_disabled, use_focus_provider, FocusState,
};
...
pub fn RadioGroup(props: RadioGroupProps) -> Element {
    ...
    rsx! {
        div {
            role: "radiogroup",
            class: format!("radio-group flex {orientation_class}"),
...
pub fn RadioItem(props: RadioItemProps) -> Element {
    ...
    // 根据状态构建radio圆形按钮的样式
    let radio_circle_classes = if (ctx.disabled)() || (props.disabled)() {
        // 禁用状态
        if checked() {
            "w-4 h-4 rounded-full border-4 border-gray-400 bg-white opacity-50 cursor-not-allowed shrink-0"
        } else {
            "w-4 h-4 rounded-full border border-gray-400 bg-white opacity-50 cursor-not-allowed shrink-0"
        }
    } else if checked() {
        // 选中状态
        "w-4 h-4 rounded-full border-4 border-blue-500 bg-white cursor-pointer shrink-0"
    } else {
        // 未选中状态
        "w-4 h-4 rounded-full border border-gray-400 bg-white cursor-pointer shrink-0"
    };
    ...
    rsx! {
        button {
            role: "radio",
            id: props.id,
            class: "flex flex-row items-center p-0 border-none bg-transparent text-gray-700 dark:text-gray-300 text-sm gap-3 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2",
            tabindex: tab_index,
            type: "button",

            aria_checked: checked,
            "data-state": if checked() { "checked" } else { "unchecked" },
            "data-disabled": if (ctx.disabled)() || (props.disabled)() { "true" } else { "false" },
