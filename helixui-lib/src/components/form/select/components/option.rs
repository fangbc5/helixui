...
use crate::{
    components::focus::use_focus_controlled_item,
    components::{use_effect, use_effect_cleanup, use_id_or, use_unique_id},
};
use dioxus::html::input_data::MouseButton;
use dioxus::prelude::*;

use super::super::context::{OptionState, SelectContext, SelectOptionContext};
use super::super::context::{RcPartialEqValue, SelectListContext};
    ...
    let text_value = use_memo(move || match (props.text_value)() {
        ...
        None => {
            ...
            as_any
                ...
                .unwrap_or_else(|| {
                    eprintln!("SelectOption with non-string types requires text_value to be set");
                    String::new()
                })
        }
    });
    ...
    rsx! {
        if render() {
            div {
                ...
                ..props.attributes,
                // 渲染 children（文本和 SelectItemIndicator）
                // 由于父容器使用 justify-between，文本会在左侧，SelectItemIndicator 会在右侧
                {props.children}
            }
        }
    }
