...
use crate::components::feedback::portal::{use_portal, PortalIn, PortalOut};
    ...
    rsx! {
        ...
        PortalIn { portal,
            div {
                ...
                tabindex: "-1",
                class: "toast-container fixed z-[9999] right-5 bottom-5 max-w-[350px] group",
                ...
                ol {
                    class: "flex flex-col-reverse p-0 m-0 gap-3",
                    // Render only the last 3 toasts for stacking effect
...
pub fn Toast(props: ToastProps) -> Element {
    ...
    if !props.permanent && props.duration.is_some() {
        ...
        let remove_toast = ctx.remove_toast;
        use_effect(move || {
            let timeout = use_timeout(duration, move |()| remove_toast.call(toast_id));
            timeout.action(());
        });
    }

    // 计算CSS变量值
    let toast_hidden = (props.index as f32 - 2.0).max(0.0).min(1.0);
    let scale_x = 1.0 - (props.index as f32 * 0.05);
    let scale_y = 1.0 - (props.index as f32 * 0.02);
    let brightness = 0.5 + 0.5 * (1.0 - (props.index as f32 + 1.0) / 4.0);
    let opacity = 1.0 - toast_hidden;
    ...
    rsx! {
        div {
            ...
            style: format!(
                "--toast-index: {}; --toast-hidden: {}; z-index: calc(var(--toast-count) - var(--toast-index)); opacity: {}; transform: scale({}, {}); filter: brightness({}); margin-top: -4rem;",
                props.index, toast_hidden, opacity, scale_x, scale_y, brightness
            ),

            div {
                class: "flex-1 mr-3 transition-all duration-200 flex flex-col justify-center min-w-0",
                role: "alert",
                aria_atomic: "true",

                div {
                    id: label_id,
                    class: "font-semibold break-words leading-tight",
                    {props.title.clone()}
                }
                if let Some(description) = &props.description {
                    div {
                        id: description_id.clone(),
                        class: "text-sm mt-1 break-words leading-relaxed",
                        {description.clone()}
                    }
                }
            }
            ...
            button {
                class: "self-center p-0 m-0 bg-transparent border-0 cursor-pointer text-lg leading-none opacity-70 hover:opacity-100 flex-shrink-0",
                aria_label: "close",
                type: "button",
                onclick: move |e| {
